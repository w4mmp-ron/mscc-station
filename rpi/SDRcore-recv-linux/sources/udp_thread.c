...
socklen_t slen = (socklen_t)sizeof(si_ms_sdr);
...
int G_Smoothing = 2;
uint8_t G_tx_mode = 0;
uint8_t G_Panadapter_Blocks = 2;
/* Pan bins across 72 kHz: 800 (Normal), 1600 (High), 3200 (Max). */
int G_Panadapter_Pixels = 800;
...
uint8_t G_Volume_ATTN = 0;
uint8_t G_Digital_Volume_ATTN = 0;
uint8_t G_Pause_Panadapter = FALSE;
uint8_t G_Pause_Panadapter_Reload = FALSE;
int G_Pause_Panadapter_Cycles = 0;
uint8_t G_Pan_Flush_History = FALSE;

/* Band / large LO: longer blank. Small steps: brief blank so live glitch is not painted. */
#define PAN_PAUSE_LARGE_LO_FRAMES  20
#define PAN_PAUSE_SMALL_STEP_FRAMES 5

static void Trigger_Panadapter_Pause(int frames) {
    G_Pause_Panadapter = TRUE;
    G_Pause_Panadapter_Reload = TRUE;
    G_Pause_Panadapter_Cycles = frames;
    G_Pan_Flush_History = TRUE;
}
...
/*int Setup_UDP(void) {
    ...
    if ((sdrcore_s = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) == INVALID_SOCKET) {
        print_time();
        fprintf(G_fp_logfile, "[%d] UDP Thread . SDRcore-recv Create Socket: Failed. Error Code : %s\n", line_number++, strerror(errno));
        status = 1;
    } else {
        print_time();
        fprintf(G_fp_logfile, "[%d] UDP Thread . SDRcore-recv Socket Created\n", line_number++);
    }
    ...
    if (G_SDRcore_port == 0) {
        print_time();
        fprintf(G_fp_logfile, "[%d] UDP Thread . Invalid DLL Port. Using default: %d\n", line_number++, SDRCORE_RECV_PORT);
    ...
    if (bind(sdrcore_s, (struct sockaddr*) &si_me, sizeof (si_me)) != 0) {
        print_time();
        fprintf(G_fp_logfile, "[%d] UDP Thread . Bind Failed. Error Code : %s\n", line_number++, strerror(errno));
        status = 1;
    }
    ...
    if (ms_sdr_port == 0) {
        print_time(0);
        fprintf(G_fp_logfile, "[%d] Command_Interface . Invalid MS-SDR Port. Using default: %d\n", line_number++, MS_SDR_PORT);
...
int Setup_UDP(void) {
    ...
    si_ms_sdr.sin_family = AF_INET;
    si_ms_sdr.sin_port = htons(ms_sdr_port);
    if (inet_pton(AF_INET, SERVER, &si_ms_sdr.sin_addr) != 1) {
        print_time();
        fprintf(G_fp_logfile, "[%d] Setup_UDP -> inet_pton(%s) failed\n", line_number++, SERVER);
        si_ms_sdr.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
    }
    print_time();
    fprintf(G_fp_logfile,
        "[%d] Setup_UDP -> listen :%d, ms-sdr %s:%d, status=%d\n",
        line_number++, G_SDRcore_port, SERVER, ms_sdr_port, status);
    fflush(G_fp_logfile);
    (void)address;
    return status;
}
...
void set_cw_bandwidth(int cw_size, int cw_center) {
    switch (cw_size) { //Width of the CW filter index. 0. 200Hz. 1 . 400Hz. 2. 1.8KHz
        case 0: //200Hz filter width
            switch (cw_center) { //CW PITCH - Center of the filter index.  
                case 0: //400Hz
                    G_cw_low_cut = 300.0f;
    ...
    print_time();
    fprintf(G_fp_logfile, "[%d] set_cw_bandwidth.Filter cw_size: %d, cw_center: %d\n", line_number++, cw_size, cw_center);
}
...
void *UDP_Thread(void *my_param) {
    ...
    uint8_t invalid_cw_pitch = FALSE;
    long delta_freq = 0;
    long previous_freq = 0;
...
#pragma pack(1)
    ...
    initAGC(1000.0f);
    /* I/Q offsets: only $HOME/.local/mscc/recv-iq.ini (no rev-MKII/ tree) */
    ...
    while (G_all_threads_run) {
        ...
        socklen_t peer_len = (socklen_t)sizeof(si_peer);

        memset(&si_peer, 0, sizeof(si_peer));
        recv_len = recvfrom(sdrcore_s, buf, BUFLEN, 0, (struct sockaddr *)&si_peer, &peer_len);
        if (recv_len == SOCKET_ERROR || recv_len < 1) {
            if (!G_all_threads_run)
                break;
            print_time();
        ...
        switch (opcode) {
        case CMD_SET_NR:
            /* Payload 0 = OFF; any other value = ON with level = payload.
             * Prefer short (s_opcode_data) so levels > 255 work if sent as int16. */
            print_time();
            fprintf(G_fp_logfile, "[%d] UDP Thread. CMD_SET_NR. payload: %d (byte %u)\n",
                line_number++, (int)s_opcode_data, (unsigned)t_opcode_data);
            if (s_opcode_data == 0) {
                nrstate.enabled = 0;
                nrstate.level = 0;
                denoise_reset();
                ...
                nrstate.enabled = 1;
                nrstate.level = (int)s_opcode_data;
            }
            fprintf(G_fp_logfile, "[%d] UDP Thread. CMD_SET_NR. enabled=%u level=%d\n",
                line_number++, (unsigned)nrstate.enabled, nrstate.level);
            break;

        case CMD_SET_MAIN_FREQ:
            op_code_data_32 = (int*)&buf[1];
            memcpy(&i_opcode_data, op_code_data_32, 4);
            delta_freq = (labs)(previous_freq - i_opcode_data);
            if (previous_freq != 0 && i_opcode_data != previous_freq) {
                if (delta_freq >= 100000) {
                    /* Large LO step → Si5351 PLL reset */
                    Trigger_Panadapter_Pause(PAN_PAUSE_LARGE_LO_FRAMES);
                } else {
                    /* 1 kHz / slow steps: brief blank + flush (avoids left-side smear) */
                    Trigger_Panadapter_Pause(PAN_PAUSE_SMALL_STEP_FRAMES);
                }
            }
            ...
            case CMD_SET_AUDIO_DEVICE:
                ...
                case REMOTE_DIGITAL_AUDIO: {
                    ...
                    if (dig < 0 || dig >= MAX_OUTPUT_DEVICES || dig == NO_OUTPUT_DEVICE) {
                        print_time();
                        fprintf(G_fp_logfile,
                            "[%d] UDP Thread. DIGITAL: no matched digi speaker "
                            "(set digital-speaker.ini e.g. VirtualA or MSCC_Cable_Out) — reopen operator\n",
                            line_number++);
                    ...
                    print_time();
                    fprintf(G_fp_logfile,
                        "[%d] UDP Thread. DIGITAL open pa=%d ch=2 name='%s'\n",
                        line_number++,
                        G_digital_output_devices[dig].device_index,
                        G_digital_output_devices[dig].name);
                    stream_status = manage_stream(1, G_digital_output_devices[dig].device_index, 2);
                    G_recv_audio_mode = t_opcode_data;
                    if (stream_status != 0) {
                        print_time();
                        fprintf(G_fp_logfile,
                            "[%d] UDP Thread. DIGITAL open FAILED status=%d (no operator fallback)\n",
                            line_number++, stream_status);
                    }
                    print_time();
                    fprintf(G_fp_logfile,
                        "[%d] UDP Thread. CMD_SET_AUDIO_DEVICE %s (mode=%d) VirtualA open status=%d\n",
                        line_number++,
                        t_opcode_data == REMOTE_DIGITAL_AUDIO ? "REMOTE_DIGITAL" : "DIGITAL",
                        (int)t_opcode_data, stream_status);
                    break;
                }
                case OPERATOR_AUDIO:
                case REMOTE_AUDIO: {
            ...
            case CMD_SET_DIGITAL_VOLUME_ATTN:
                ...
                G_t_volumeLevel = master_side_tone_volume * volume_attn;
                print_time();
                fprintf(G_fp_logfile, "[%d] UDP Thread. CMD_SET_DIGITAL_VOLUME_ATTN. volume_attn: %f, master_volume: %f, G_volumeLevel: %f\n",
                    line_number++, volume_attn, master_volume, G_volumeLevel);
                break;
            ...
            case CMD_SET_PCB_VERSION:
                ...
                if (status == 1) {
                    print_time();
                    fprintf(G_fp_logfile, "[%d] UDP_Thread. init_IQ_structure. FINISHED \n", line_number++);
                } else {
                    print_time();
            ...
            case CMD_GET_SET_PANADAPTER_REFRESH:
                /* MSCC-Net9: index 0/1/2 → pan resolution 800/1600/3200 bins.
                 * Legacy refresh values 3–10 still adjust G_Panadapter_Blocks only. */
                if (t_opcode_data <= 2) {
                    static const int pix_table[3] = { 800, 1600, 3200 };
                    static const char *res_name[3] = { "Normal", "High", "Max" };
                    int new_pix = pix_table[t_opcode_data];
                    int old_pix = G_Panadapter_Pixels;
                    if (new_pix > MAX_PIXELS) new_pix = MAX_PIXELS;
                    if (new_pix != G_Panadapter_Pixels) {
                        /* Drop in-flight pan first so DSP does not write while we clear */
                        panbuffer.panReady = 0;
                        G_Panadapter_Pixels = new_pix;
                        memset(panbuffer.Y, 0, sizeof(panbuffer.Y));
                        G_Pan_Flush_History = TRUE;
                    }
                    print_time();
                    fprintf(G_fp_logfile,
                        "[%d] UDP Thread. PAN RESOLUTION %s → %d bins (index %d, %s) "
                        "was %d  segments=%d  Ybuf=%u bytes\n",
                        line_number++,
                        (old_pix != G_Panadapter_Pixels) ? "CHANGED" : "unchanged",
                        G_Panadapter_Pixels, t_opcode_data,
                        res_name[t_opcode_data],
                        old_pix,
                        G_Panadapter_Pixels / MAX_X,
                        (unsigned)sizeof(panbuffer.Y));
                    fflush(G_fp_logfile);
                } else if (t_opcode_data >= 3 && t_opcode_data <= 10) {
                    uint8_t old_blocks = G_Panadapter_Blocks;
                    G_Panadapter_Blocks = (uint8_t)t_opcode_data;
                    print_time();
                    fprintf(G_fp_logfile,
                        "[%d] UDP Thread. PAN REFRESH BLOCKS %s → %d (was %d) "
                        "(legacy rate, not bin resolution)\n",
                        line_number++,
                        (old_blocks != G_Panadapter_Blocks) ? "CHANGED" : "unchanged",
                        G_Panadapter_Blocks, old_blocks);
                    fflush(G_fp_logfile);
                } else {
                    print_time();
                    fprintf(G_fp_logfile,
                        "[%d] UDP Thread. PAN REFRESH ignored opcode_data=%d "
                        "(want 0..2 resolution or 3..10 blocks)\n",
                        line_number++, t_opcode_data);
                    fflush(G_fp_logfile);
                }
                break;
            ...
            case CMD_SET_CW_PITCH:
                ...
                fprintf(G_fp_logfile, "[%d] UDP Thread. CMD_SET_CW_PITCH: %d. STARTED\n", line_number++, cw_center);
                G_Pitch_changed = 1;
                invalid_cw_pitch = FALSE;
                if (cw_bw_set) {
                    switch (cw_center) {
                        case 0:
                            G_CW_pitch = 400.00;
                        ...
                        case 3:
                            G_CW_pitch = 1000.00;
                            G_side_tone_pitch = 1000.00f;
                            break;
                        default:
                            invalid_cw_pitch = TRUE;
                            break;
                    }
                    if (invalid_cw_pitch == FALSE) {
                        set_cw_bandwidth(cw_bw, cw_center);
                    } else {
                        print_time();
                        fprintf(G_fp_logfile, "[%d] UDP Thread. CMD_SET_CW_PITCH: INVALID CW PITCH: %d\n", line_number++,
                                cw_center);
                    }
                    if (!cw_mode) {
            ...
            case CMD_GET_SET_PANADAPTER_SMOOTHING:
                {
                    int old_smooth = G_Smoothing;
                    G_Smoothing = t_opcode_data + 2;
                    print_time();
                    fprintf(G_fp_logfile,
                        "[%d] UDP Thread. PAN SMOOTHING %s → %d (raw index %d, was %d)\n",
                        line_number++,
                        (old_smooth != G_Smoothing) ? "CHANGED" : "unchanged",
                        G_Smoothing, t_opcode_data, old_smooth);
                    fflush(G_fp_logfile);
                }
                break;

            case CMD_GET_SET_PANADAPTER_GAIN:
                /* Often client display only; log so we see if GUI sends it to the core. */
                print_time();
                fprintf(G_fp_logfile,
                    "[%d] UDP Thread. PAN GAIN cmd received raw=%d s_data=%d "
                    "(recv core does not apply display grid gain)\n",
                    line_number++, t_opcode_data, s_opcode_data);
                fflush(G_fp_logfile);
                break;
            ...
            case CMD_GET_SET_PANADAPTER_BASE:
                print_time();
                fprintf(G_fp_logfile,
                    "[%d] UDP Thread. PAN BASE/grid cmd received raw=%d s_data=%d "
                    "(recv core does not apply display grid base — usually client-only)\n",
                    line_number++, t_opcode_data, s_opcode_data);
                fflush(G_fp_logfile);
                break;
            ...
            case CMD_DELETE_SDRCORE_INIT:
                print_time();
                fprintf(G_fp_logfile, "[%d] UDP Thread. CMD_DELETE_SDRCORE_INIT. NOOP \n", line_number++);
                //delete_ini_file();
                break;

            case CMD_SET_KEEP_ALIVE:
                G_keep_alive++;
                if ((G_keep_alive % 10) == 1) {
                    print_time();
                    fprintf(G_fp_logfile,
                        "[%d] UDP Thread. CMD_SET_KEEP_ALIVE from %s:%u count=%lld\n",
                        line_number++,
                        inet_ntoa(si_peer.sin_addr),
                        (unsigned)ntohs(si_peer.sin_port),
                        (long long)G_keep_alive);
                    fflush(G_fp_logfile);
                }
                break;
            ...
            case CMD_SET_SDR_CORE_BAND:
                print_time();
                fprintf(G_fp_logfile, "[%d] UDP Thread. CMD_SET_SDR_CORE_BAND. STARTED\n", line_number++);
                operating_band = s_opcode_data;
                if (previous_operating_band != operating_band) {
                    /* Band change always implies large LO / filter step — blank spectrum */
                    Trigger_Panadapter_Pause(PAN_PAUSE_LARGE_LO_FRAMES);
                    Init_IQ_structure();
                    ...
                    print_time();
                    fprintf(G_fp_logfile, "[%d] UDP Thread. CMD_SET_SDR_CORE_BAND. band NOT Changed. FINISHED band: %d\n",
                            line_number++, operating_band);
                }
                break;
            ...
            case CMD_SET_IQ_BAND:
                ...
                if (sendto(ms_sdr_s, (char *) &iq_buffer, sizeof (iq_buffer), 0,
                    ...
                    print_time();
                    fprintf(G_fp_logfile, "[%d] UDP Thread. CMD_SET_IQ_BAND. sentto. error code : %s. FINSHED\n",
                            line_number++, strerror(errno));
                }
                print_time();
                fprintf(G_fp_logfile, "[%d] UDP Thread. CMD_SET_IQ_BAND. band: %d. FINISHED\n",
                        line_number++, iq_band);
                break;

            case CMD_SET_IQ_ALL_BANDS:
                print_time();
                fprintf(G_fp_logfile, "[%d] UDP Thread. CMD_SET_IQ_DEFAULTS \n", line_number++);
                status = IQ_Set_All_Band(iq_offset);
                Init_IQ_structure();
                break;
            ...
            case CMD_SET_CW_BW:
                ...
                print_time();
                fprintf(G_fp_logfile, "[%d] UDP Thread. CMD_SET_CW_BW. cw_bw_received: %d\n", line_number++, cw_bw_received);
                cw_bw_set = TRUE;
                ...
                print_time();
                fprintf(G_fp_logfile, "[%d] UDP Thread. CMD_SET_CW_BW. cw_bw: %d,New filter . Low: %f, High: %f, cw_mode: %d\n",
