...
I2C_DISPLAY_BACKUP_STRUCT I2C_DISPLAY_backup =
{
    I2C_DISPLAY_DISABLE,

#if (I2C_DISPLAY_FF_IMPLEMENTED)
    I2C_DISPLAY_DEFAULT_XCFG,
    I2C_DISPLAY_DEFAULT_CFG,
    I2C_DISPLAY_DEFAULT_ADDR,
    LO8(I2C_DISPLAY_DEFAULT_DIVIDE_FACTOR),
    HI8(I2C_DISPLAY_DEFAULT_DIVIDE_FACTOR),
#else  /* (I2C_DISPLAY_UDB_IMPLEMENTED) */
    I2C_DISPLAY_DEFAULT_CFG,
#endif /* (I2C_DISPLAY_FF_IMPLEMENTED) */

#if (I2C_DISPLAY_TIMEOUT_ENABLED)
    I2C_DISPLAY_DEFAULT_TMOUT_PERIOD,
    I2C_DISPLAY_DEFAULT_TMOUT_INTR_MASK,
#endif /* (I2C_DISPLAY_TIMEOUT_ENABLED) */
};

#if ((I2C_DISPLAY_FF_IMPLEMENTED) && (I2C_DISPLAY_WAKEUP_ENABLED))
    volatile uint8 I2C_DISPLAY_wakeupSource;
#endif /* ((I2C_DISPLAY_FF_IMPLEMENTED) && (I2C_DISPLAY_WAKEUP_ENABLED)) */


/*******************************************************************************
* Function Name: I2C_DISPLAY_SaveConfig
********************************************************************************
*
* Summary:
...
*
* Global Variables:
*  I2C_DISPLAY_backup - The global variable used to save the component
...
{
#if (I2C_DISPLAY_FF_IMPLEMENTED)
    #if (I2C_DISPLAY_WAKEUP_ENABLED)
        uint8 intState;
    #endif /* (I2C_DISPLAY_WAKEUP_ENABLED) */

    /* Store registers before enter low power mode */
    I2C_DISPLAY_backup.cfg     = I2C_DISPLAY_CFG_REG;
    I2C_DISPLAY_backup.xcfg    = I2C_DISPLAY_XCFG_REG;
    I2C_DISPLAY_backup.addr    = I2C_DISPLAY_ADDR_REG;
    I2C_DISPLAY_backup.clkDiv1 = I2C_DISPLAY_CLKDIV1_REG;
    I2C_DISPLAY_backup.clkDiv2 = I2C_DISPLAY_CLKDIV2_REG;

#if (I2C_DISPLAY_WAKEUP_ENABLED)
    /* Disable master */
    I2C_DISPLAY_CFG_REG &= (uint8) ~I2C_DISPLAY_ENABLE_MASTER;

    /* Enable backup regulator to keep block powered in low power mode */
    intState = CyEnterCriticalSection();
    I2C_DISPLAY_PWRSYS_CR1_REG |= I2C_DISPLAY_PWRSYS_CR1_I2C_REG_BACKUP;
    ...
    */
    ...
    I2C_DISPLAY_XCFG_REG |= I2C_DISPLAY_XCFG_FORCE_NACK;
    while (0u == (I2C_DISPLAY_XCFG_REG & I2C_DISPLAY_XCFG_RDY_TO_SLEEP))
    {
    }

    /* Setup wakeup interrupt */
    I2C_DISPLAY_DisableInt();
    (void) CyIntSetVector(I2C_DISPLAY_ISR_NUMBER, &I2C_DISPLAY_WAKEUP_ISR);
    I2C_DISPLAY_wakeupSource = 0u;
    I2C_DISPLAY_EnableInt();
#endif /* (I2C_DISPLAY_WAKEUP_ENABLED) */

#else
    /* Store only address match bit */
    I2C_DISPLAY_backup.control = (I2C_DISPLAY_CFG_REG & I2C_DISPLAY_CTRL_ANY_ADDRESS_MASK);
#endif /* (I2C_DISPLAY_FF_IMPLEMENTED) */

#if (I2C_DISPLAY_TIMEOUT_ENABLED)
    I2C_DISPLAY_TimeoutSaveConfig();
#endif /* (I2C_DISPLAY_TIMEOUT_ENABLED) */
}


/*******************************************************************************
* Function Name: I2C_DISPLAY_Sleep
********************************************************************************
*
* Summary:
...
void I2C_DISPLAY_Sleep(void) 
{
    ...
    I2C_DISPLAY_backup.enableState = I2C_DISPLAY_DISABLE;

    #if (I2C_DISPLAY_TIMEOUT_ENABLED)
        I2C_DISPLAY_TimeoutStop();
    #endif /* (I2C_DISPLAY_TIMEOUT_ENABLED) */

#else
    /* Store enable state */
    I2C_DISPLAY_backup.enableState = (uint8) I2C_DISPLAY_IS_ENABLED;

    if (0u != I2C_DISPLAY_backup.enableState)
    {
        I2C_DISPLAY_Stop();
    }
#endif /* (I2C_DISPLAY_WAKEUP_ENABLED) */

    I2C_DISPLAY_SaveConfig();
}


/*******************************************************************************
* Function Name: I2C_DISPLAY_RestoreConfig
********************************************************************************
*
* Summary:
*  The Enable wakeup from Sleep Mode selection influences this function
...
*
* Side Effects:
*  Calling this function before I2C_DISPLAY_SaveConfig() or
*  I2C_DISPLAY_Sleep() will lead to unpredictable results.
*
*******************************************************************************/
void I2C_DISPLAY_RestoreConfig(void) 
{
    ...
    if (I2C_DISPLAY_CHECK_PWRSYS_I2C_BACKUP)
    /* Low power mode was Sleep - backup regulator is enabled */
    {
        /* Enable backup regulator in active mode */
        intState = CyEnterCriticalSection();
        I2C_DISPLAY_PWRSYS_CR1_REG &= (uint8) ~I2C_DISPLAY_PWRSYS_CR1_I2C_REG_BACKUP;
        CyExitCriticalSection(intState);

        /* Restore master */
        I2C_DISPLAY_CFG_REG = I2C_DISPLAY_backup.cfg;
    }
    else
    /* Low power mode was Hibernate - backup regulator is disabled. All registers are cleared */
    {
        ...
        intState = CyEnterCriticalSection();
        I2C_DISPLAY_ACT_PWRMGR_REG  &= (uint8) ~I2C_DISPLAY_ACT_PWR_EN;
        I2C_DISPLAY_STBY_PWRMGR_REG &= (uint8) ~I2C_DISPLAY_STBY_PWR_EN;
        CyExitCriticalSection(intState);

        /* Enable component in I2C_Wakeup() after register restore */
        I2C_DISPLAY_backup.enableState = I2C_DISPLAY_ENABLE;
    #endif /* (I2C_DISPLAY_WAKEUP_ENABLED) */

        /* Restore component registers after Hibernate */
        I2C_DISPLAY_XCFG_REG    = I2C_DISPLAY_backup.xcfg;
        I2C_DISPLAY_CFG_REG     = I2C_DISPLAY_backup.cfg;
        I2C_DISPLAY_ADDR_REG    = I2C_DISPLAY_backup.addr;
        I2C_DISPLAY_CLKDIV1_REG = I2C_DISPLAY_backup.clkDiv1;
        I2C_DISPLAY_CLKDIV2_REG = I2C_DISPLAY_backup.clkDiv2;
    }

#if (I2C_DISPLAY_WAKEUP_ENABLED)
    I2C_DISPLAY_DisableInt();
    (void) CyIntSetVector(I2C_DISPLAY_ISR_NUMBER, &I2C_DISPLAY_ISR);
    if (0u != I2C_DISPLAY_wakeupSource)
    {
        /* Generate interrupt to process incoming transaction */
        I2C_DISPLAY_SetPendingInt();
    }
    I2C_DISPLAY_EnableInt();
#endif /* (I2C_DISPLAY_WAKEUP_ENABLED) */

#else
    I2C_DISPLAY_CFG_REG = I2C_DISPLAY_backup.control;
#endif /* (I2C_DISPLAY_FF_IMPLEMENTED) */

#if (I2C_DISPLAY_TIMEOUT_ENABLED)
    I2C_DISPLAY_TimeoutRestoreConfig();
#endif /* (I2C_DISPLAY_TIMEOUT_ENABLED) */
}


/*******************************************************************************
* Function Name: I2C_DISPLAY_Wakeup
********************************************************************************
*
* Summary:
