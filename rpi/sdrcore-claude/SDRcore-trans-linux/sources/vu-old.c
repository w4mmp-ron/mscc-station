...
void *ALC_thread(void *t) {
    ...
    print_time();
    fprintf(G_fp_logfile, "[%d] ALC_thread - Thread has started\n", line_number++);
    while (G_all_threads_run) {
        while (G_tx_mode == 1 && G_mode != 'T') {
            if (reset == TRUE) {
                zero_count = 0;
                alc = 1.0000f;
                ...
                if (sendto(ms_sdr_s, (char *) &send_buf, 5, 0, (struct sockaddr *) &si_ms_sdr, slen) == SOCKET_ERROR) {
                    print_time();
                    fprintf(G_fp_logfile, "[%d] ALC_thread -> sentto failed with error code : %d\n",
