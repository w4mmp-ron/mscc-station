...
static int sdrAudioCallback(const void *inputBuffer, void *outputBuffer,
        ...
        void *userData);
static int sdrIqOnlyCallback(const void *inputBuffer, void *outputBuffer,
        unsigned long framesPerBuffer,
        const PaStreamCallbackTimeInfo* timeInfo,
        PaStreamCallbackFlags statusFlags,
        void *userData);
static int sdrPlayOnlyCallback(const void *inputBuffer, void *outputBuffer,
        unsigned long framesPerBuffer,
        const PaStreamCallbackTimeInfo* timeInfo,
        PaStreamCallbackFlags statusFlags,
        void *userData);
...
/*
 * Dual-stream bridge (ALSA I/Q in + Pulse digi out): PortAudio rejects mixed-host
 * full duplex (-9993). I/Q callback writes stereo @ 96k; play callback reads it.
 * When operator device cannot open at 96k, play opens at device rate and Oboe
 * MultiChannelResampler converts 96k AF → play rate (digi VirtualA stays 96k path).
 */
#define DIGI_RING_FRAMES 16384
static float g_digi_ring[DIGI_RING_FRAMES * 2];
static volatile unsigned g_digi_w = 0;
static volatile unsigned g_digi_r = 0;
static int g_split_streams = 0; /* 0 = one full-duplex stream, 1 = in+out streams */
static PaStream *stream_out = NULL; /* play-only when split */
...
static void digi_ring_write(const SAMPLE *stereo, unsigned long frames)
{
    unsigned long i;
    for (i = 0; i < frames; i++) {
        unsigned next = (g_digi_w + 1u) % DIGI_RING_FRAMES;
        if (next == g_digi_r)
            break; /* full — drop rest */
        g_digi_ring[g_digi_w * 2u] = stereo[i * 2u];
        g_digi_ring[g_digi_w * 2u + 1u] = stereo[i * 2u + 1u];
        g_digi_w = next;
    }
}
...
static void play_resampler_destroy(void)
{
    if (g_play_resampler != NULL) {
        mscc_resampler_destroy(g_play_resampler);
        g_play_resampler = NULL;
    }
    g_play_rate = 0.0;
}

/*
 * Choose play sample rate for the operator/digi output device.
 * Prefer I/Q rate (96k). If Pa_IsFormatSupported fails, use device default
 * (typically 48k/44.1k), then try 48000 and 44100.
 * Returns rate > 0 always (falls back to I/Q rate as last resort).
 */
static double pick_play_sample_rate(const PaStreamParameters *out_params,
                                    const PaDeviceInfo *odi,
                                    double iq_rate)
{
    PaStreamParameters probe = *out_params;
    PaError chk;
...

    {
        ...
        for (i = 0; i < sizeof(candidates) / sizeof(candidates[0]); i++) {
            if (candidates[i] == iq_rate)
                continue;
            chk = Pa_IsFormatSupported(NULL, &probe, candidates[i]);
            if (chk == paFormatIsSupported)
                return candidates[i];
        }
    }

    /* Last resort: open attempt will use iq_rate (may still fail). */
    return iq_rate;
}

/* DSP: I/Q stereo in → stereo audio out (interleaved L/R). Safe for callback. */
static void process_iq_to_stereo(const SAMPLE *in, SAMPLE *out, unsigned long framesPerBuffer)
{
    unsigned int i;
    ...
    if (anstate.enabled)
        anotch(outcplx, (int)framesPerBuffer);
...
/* Split: I/Q capture only (ALSA Proficio). */
static int sdrIqOnlyCallback(const void *inputBuffer, void *outputBuffer,
        unsigned long framesPerBuffer,
        const PaStreamCallbackTimeInfo* timeInfo,
        PaStreamCallbackFlags statusFlags,
        void *userData) {
    static SAMPLE tmp[4096 * 2];
    (void)outputBuffer;
    (void)timeInfo;
    (void)statusFlags;
    (void)userData;
    ...
    if (inputBuffer == NULL || framesPerBuffer > 4096u)
        return paContinue;
    process_iq_to_stereo((const SAMPLE *)inputBuffer, tmp, framesPerBuffer);
    digi_ring_write(tmp, framesPerBuffer);
    return paContinue;
}

/* Split: digi/operator play only (e.g. Pulse VirtualA or non-96k phones). */
static int sdrPlayOnlyCallback(const void *inputBuffer, void *outputBuffer,
        unsigned long framesPerBuffer,
        const PaStreamCallbackTimeInfo* timeInfo,
        PaStreamCallbackFlags statusFlags,
...
void Audio_Device_Error(PaError err) {
    /*
     * Fatal audio init failure. Must not return — callers used to fall through
     * and dereference Pa_GetDeviceInfo(paNoDevice) → segfault.
     */
    G_all_threads_run = 0;
    if (G_fp_logfile != NULL) {
        ...
        print_time();
        fprintf(G_fp_logfile, "[%d] sdrcore-recv terminating (audio init failed)\n", line_number++);
        fflush(G_fp_logfile);
    }
...
int manage_stream(int start_stop, int device, int channels) {
    ...
    if (start_stop == TRUE) {
        if (started == FALSE) {
            const PaDeviceInfo *odi = Pa_GetDeviceInfo((PaDeviceIndex)device);
            const PaDeviceInfo *idi = Pa_GetDeviceInfo(inputParameters.device);
            ...
            fprintf(G_fp_logfile, "[%d] Main Thread. manage_stream. STARTED.  Device: %d, Channels %d\n",
                    line_number++, device, channels);
            if (idi)
                in_api = Pa_GetHostApiInfo(idi->hostApi);
            if (odi)
                out_api = Pa_GetHostApiInfo(odi->hostApi);
            if (idi && odi && idi->hostApi != odi->hostApi)
                different_host = 1;
            ...
            if (odi == NULL) {
                print_time();
                fprintf(G_fp_logfile, "[%d] manage_stream. invalid output device %d\n",
                    line_number++, device);
                return paInvalidDevice;
            }
            if (idi == NULL) {
                print_time();
                fprintf(G_fp_logfile, "[%d] manage_stream. invalid I/Q input device %d\n",
            ...
            /*
             * Pick play rate: prefer 96k (I/Q). Non-96k phones → device default.
             * PortAudio full duplex needs one rate; rate mismatch or mixed host
             * API forces dual streams. Resampler only when play_rate != iq_rate.
             */
            play_rate = pick_play_sample_rate(&outputParameters, odi, iq_rate);
            need_resample = (play_rate != iq_rate) ? 1 : 0;
            need_dual = (different_host || need_resample) ? 1 : 0;
            play_frames = (unsigned long)mystate.frames;
            if (need_resample && play_rate > 0.0 && iq_rate > 0.0) {
                play_frames = (unsigned long)(
                    0.5 + (double)mystate.frames * play_rate / iq_rate);
                if (play_frames < 64ul)
                    play_frames = 64ul;
                if (play_frames > 4096ul)
                    play_frames = 4096ul;
            }
            ...
            if (need_dual) {
                ...
                fprintf(G_fp_logfile,
                    ...
                    "out_dev=%d out_api='%s' out_name='%s' @%.0f%s\n",
                    line_number++,
                ...
                if (need_resample) {
                    g_play_resampler = mscc_resampler_create(
                        (int)(iq_rate + 0.5), (int)(play_rate + 0.5));
                    if (g_play_resampler == NULL) {
                        print_time();
                        fprintf(G_fp_logfile,
                            "[%d] manage_stream. dual: resampler create FAILED "
                            "(%.0f -> %.0f)\n",
                            line_number++, iq_rate, play_rate);
                ...
                err = Pa_OpenStream(&stream, &in_only, NULL, iq_rate, mystate.frames, 0,
                    sdrIqOnlyCallback, NULL);
                if (err != paNoError) {
                    print_time();
                    fprintf(G_fp_logfile,
                    ...
                    err = Pa_OpenStream(&stream_out, NULL, &out_only, play_rate,
                        play_frames, 0, sdrPlayOnlyCallback, NULL);
                    if (err != paNoError) {
                        print_time();
                        fprintf(G_fp_logfile,
                            "[%d] manage_stream. dual: play OpenStream FAILED PA %d '%s' "
                            "rate=%.0f frames=%lu\n",
                            line_number++, err, Pa_GetErrorText(err),
                            play_rate, (unsigned long)play_frames);
                        manage_stream_close_all();
                    } else {
                        ...
                        if (err != paNoError) {
                            ...
                            fprintf(G_fp_logfile,
                                ...
                                "(I/Q @%.0f + play @%.0f%s)\n",
                                line_number++, iq_rate, play_rate,
                                need_resample ? ", resample" : ", ring bypass");
                        }
                    }
                }
            } else {
                /* Same host API and both at I/Q rate — full duplex, no resampler. */
                print_time();
                fprintf(G_fp_logfile,
                    "[%d] manage_stream. FULL-DUPLEX (same API, rate=%.0f): "
                    "in_dev=%d in_api='%s' in_name='%s' | out_dev=%d out_api='%s' out_name='%s'\n",
                    line_number++,
                    iq_rate,
                    (int)inputParameters.device,
                    (in_api && in_api->name) ? in_api->name : "?",
                    ...
                    odi->name ? odi->name : "?");
                play_resampler_destroy();
                err = Pa_OpenStream(&stream, &inputParameters, &outputParameters, iq_rate,
                ...
                if (err != paNoError) {
                    ...
                    if (err != paNoError) {
                        print_time();
                        fprintf(G_fp_logfile,
                            "[%d] manage_stream. full-duplex StartStream FAILED PA %d '%s'\n",
                            line_number++, err, Pa_GetErrorText(err));
                        manage_stream_close_all();
                    } else {
                        g_split_streams = 0;
        ...
        if (started == TRUE) {
            ...
            print_time();
            fprintf(G_fp_logfile,
                "[%d] Main Thread. manage_stream. Stopping Stream. New device: %d, New Channels %d "
                "(split=%d play_rate=%.0f resample=%d)\n",
                line_number++, device, channels, g_split_streams, g_play_rate,
                g_play_resampler != NULL ? 1 : 0);
            manage_stream_close_all();
            started = FALSE;
            Sleep(80); /* settle before reopen */
        } else {
            print_time();
...
int main(int argc, char **argv) {
    ...
    /* Optional: post-DSP AF → Windows MsccRemotePhones (MSA1 UDP) */
    remote_phones_init();
    ...
    /* I/Q capture is always Multus/Proficio radio audio — NOT operator-microphone.ini */
    /******************************************************************/
    err = Pa_Initialize();
    ...
    fprintf(G_fp_logfile, "[%d] main -> PortAudio Version text: '%s'\n", line_number++,
            Pa_GetVersionInfo() ? Pa_GetVersionInfo()->versionText : "(null)");

    /*
     * Host API preference:
     *   Windows: MME
     *   Linux/Pi: ALSA first (low latency Multus sound device), else Pulse/JACK/any
     */
    hostApiCount = Pa_GetHostApiCount();
        ...
        for (j = 0; j < hostApiCount; j++) {
            ...
            if (lpApiInfo && lpApiInfo->name &&
                (strstr(lpApiInfo->name, "Pulse") || strstr(lpApiInfo->name, "JACK") ||
                 strstr(lpApiInfo->name, "OSS") || strstr(lpApiInfo->name, "Core"))) {
                apiTypeId = j;
                break;
            }
        }
    ...
    recordDevice = paNoDevice;
    {
        /*
         * Score candidates so Multus/Proficio wins over generic USB, and
         * ALSA hw: devices beat default/pulse/dsnoop wrappers.
         * PortAudio on Linux often names Proficio "USB Audio Device" not "Proficio".
         * aplay -l can show card "Proficio" while Pa name does not — use ini + heuristics.
         */
        int best_score = -1;
        const char *sel_reason = NULL;
        devCount = Pa_GetDeviceCount();
        print_time();
        fprintf(G_fp_logfile, "[%d] main -> Device count: %d\n", line_number++, (int)devCount);
        print_time();
        fprintf(G_fp_logfile,
            "[%d] main -> I/Q capture: Multus/Proficio radio audio only "
            "(not operator mic / digi cable)\n",
            line_number++);

        for (j = 0; j < devCount; j++) {
            int in_ch, out_ch, score;
            int is_rig, is_usb_hw, is_virtual, is_wrapper;
            const char *n;
            lpInfo = Pa_GetDeviceInfo((PaDeviceIndex)j);
            if (lpInfo == NULL || lpInfo->name == NULL)
                continue;
            n = lpInfo->name;
            in_ch = (int)lpInfo->maxInputChannels;
            out_ch = (int)lpInfo->maxOutputChannels;
            print_time();
            fprintf(G_fp_logfile,
            ...
            /* Digi / virtual cables / operator path — never I/Q */
            is_virtual =
                (strstr(n, "Loopback") != NULL || strstr(n, "aloop") != NULL ||
                 strstr(n, "MSCC_Cable") != NULL || strstr(n, "MSCC Cable") != NULL ||
                 strstr(n, "mscc_out") != NULL || strstr(n, "mscc_in") != NULL ||
                 strstr(n, "VB-Audio") != NULL || strstr(n, "CABLE") != NULL ||
                 strstr(n, "Null") != NULL ||
                 strstr(n, "audioinjector") != NULL || strstr(n, "AudioInjector") != NULL ||
                 (strncmp(n, "mscc", 4) == 0 || strncmp(n, "MSCC", 4) == 0));
            is_wrapper =
                (strstr(n, "sysdefault") != NULL || strstr(n, "dmix") != NULL ||
                 strstr(n, "dsnoop") != NULL || strstr(n, "front:") != NULL ||
                 strstr(n, "surround") != NULL || strstr(n, "iec958") != NULL ||
                 strstr(n, "hdmi") != NULL || strstr(n, "HDMI") != NULL ||
                 strcmp(n, "default") == 0 || strstr(n, "pulse") != NULL ||
                 strstr(n, "speex") != NULL || strstr(n, "upmix") != NULL ||
                 strstr(n, "vdownmix") != NULL);
            if (is_virtual || is_wrapper || in_ch > 8)
                continue;

            /* Transceiver I/Q only: Multus / Proficio product name */
            is_rig = (strstr(n, "Multus") != NULL || strstr(n, "multus") != NULL ||
                      strstr(n, "Proficio") != NULL || strstr(n, "proficio") != NULL ||
                      strstr(n, "PROFICIO") != NULL);
            /* Fallback: USB Audio hw: (Proficio when product string missing) */
            is_usb_hw =
                ((strstr(n, "USB") != NULL || strstr(n, "usb") != NULL) &&
                 strstr(n, "hw:") != NULL &&
                 (strstr(n, "Audio") != NULL || strstr(n, "CODEC") != NULL ||
                  strstr(n, "Codec") != NULL));

            /* Fixed priority — never scale by channel count (loopback 32/128) */
            score = 0;
            if (is_rig)
                score = 10000;
            else if (is_usb_hw && in_ch >= 2)
                score = 2000;
            else if (is_usb_hw && in_ch >= 1)
                score = 1500;
            if (score <= 0)
                continue;
            if (lpInfo->hostApi == apiTypeId)
            ...
            if (in_ch >= 2)
                score += 20;
            if (out_ch >= 1 && out_ch <= 8)
                score += 5;

            print_time();
            fprintf(G_fp_logfile,
                "[%d] I/Q candidate score=%d in=%d out=%d api=%d name='%s'%s%s\n",
                line_number++, score, in_ch, out_ch, lpInfo->hostApi, n,
                is_rig ? " [rig]" : "", is_usb_hw ? " [usb]" : "");

            if (score > best_score) {
                best_score = score;
                recordDevice = (PaDeviceIndex)j;
                sel_reason = is_rig ? "Proficio/Multus I/Q radio" : "USB audio I/Q fallback";
                print_time();
                fprintf(G_fp_logfile,
                    "[%d] I/Q capture BEST so far: index=%d score=%d reason=%s name='%s'\n",
                    line_number++, j, score, sel_reason, n);
            }
        }
        if (recordDevice != paNoDevice && sel_reason != NULL) {
            print_time();
            fprintf(G_fp_logfile,
                "[%d] *** I/Q CAPTURE SELECTED: PaIndex=%d reason=%s ***\n",
                line_number++, (int)recordDevice, sel_reason);
        }
    }

    /* Fallbacks if ini names did not match any speaker */
    if (G_output_device_index == NO_OUTPUT_DEVICE) {
        PaDeviceIndex def_out = Pa_GetDefaultOutputDevice();
        if (def_out != paNoDevice) {
            for (j = 0; j < num_output_devices_found; j++) {
                if (G_output_devices[j].device_index == (int)def_out) {
                    G_output_device_index = j;
                    break;
                }
            }
            if (G_output_device_index == NO_OUTPUT_DEVICE && num_output_devices_found > 0)
                G_output_device_index = 0;
            print_time();
            fprintf(G_fp_logfile,
                "[%d] main. operator-speaker.ini unmatched (Windows name?) — "
                "using output slot %d name='%s'\n",
                line_number++, G_output_device_index,
                G_output_device_index != NO_OUTPUT_DEVICE &&
                    G_output_device_index < num_output_devices_found
                    ? G_output_devices[G_output_device_index].name
                    : "?");
        }
    }
    /* Do NOT fall back to digital slot 0 (often HDMI / first ALSA out).
     * D mode requires a real match to digital-speaker.ini
     * (e.g. VirtualA via Pulse, or MSCC_Cable_Out via ALSA). */
    if (G_digital_output_device_index == NO_OUTPUT_DEVICE) {
        print_time();
        fprintf(G_fp_logfile,
            "[%d] main. digital speaker NOT matched (ini='%s'). "
            "D mode will fail until that name is visible to PortAudio "
            "(Pulse VirtualA or ALSA MSCC_Cable_Out).\n",
            line_number++,
            G_digital_audio_device[0] ? G_digital_audio_device : "(empty)");
        fflush(G_fp_logfile);
    } else {
        const PaDeviceInfo *ddi =
            Pa_GetDeviceInfo((PaDeviceIndex)G_digital_output_devices[G_digital_output_device_index].device_index);
        const PaHostApiInfo *dhai = (ddi != NULL) ? Pa_GetHostApiInfo(ddi->hostApi) : NULL;
        print_time();
        fprintf(G_fp_logfile,
            "[%d] main. digital speaker: slot=%d pa=%d ch=%d api='%s' name='%s'\n",
            line_number++,
            G_digital_output_device_index,
            G_digital_output_devices[G_digital_output_device_index].device_index,
            G_digital_output_devices[G_digital_output_device_index].num_channels,
            (dhai && dhai->name) ? dhai->name : "?",
            G_digital_output_devices[G_digital_output_device_index].name);
        fflush(G_fp_logfile);
    }

    if (recordDevice == paNoDevice) {
        print_time();
        fprintf(G_fp_logfile,
            "[%d] No Proficio/Multus I/Q capture device found.\n"
            "     Need PortAudio name containing Proficio/Multus (radio I/Q), not operator mic or MSCC cable.\n"
            "     Check: arecord -l ; device list above must include 'Proficio: USB Audio (hw:...)'\n",
            line_number++);
        fflush(G_fp_logfile);
    ...
    if (inputParameters.device == paNoDevice ||
        ...
        print_time();
        fprintf(G_fp_logfile, "[%d] Error: I/Q capture device not usable.\n", line_number++);
        Audio_Device_Error(err);
    }
    {
        const PaDeviceInfo *cap = Pa_GetDeviceInfo(inputParameters.device);
        /* Proficio I/Q is always stereo */
        inputParameters.channelCount = 2;
        inputParameters.sampleFormat = PA_SAMPLE_TYPE;
        inputParameters.suggestedLatency =
            cap ? cap->defaultLowInputLatency : 0.025f;
        inputParameters.hostApiSpecificStreamInfo = NULL;
        print_time();
        fprintf(G_fp_logfile,
            "[%d] main. I/Q CAPTURE open: PaIndex=%d ch=%d name='%s' rate=%d\n",
            line_number++, (int)recordDevice, inputParameters.channelCount,
            cap->name ? cap->name : "?", (int)mystate.samplerate);
        fflush(G_fp_logfile);
    }

    if (G_digital_output_device_index == NO_OUTPUT_DEVICE) {
        print_time();
        fprintf(G_fp_logfile, "[%d] main. Warning: no digital output device matched\n",
            line_number++);
    }
    if (G_output_device_index == NO_OUTPUT_DEVICE) {
        print_time();
        fprintf(G_fp_logfile, "[%d] main. Error: no operator speaker / output device\n",
            line_number++);
        Audio_Device_Error(err);
    } else {
        print_time();
        fprintf(G_fp_logfile,
            "[%d] main. Opening FULL-DUPLEX stream:\n"
            "     CAPTURE  PaIndex=%d ch=%d name='%s'\n"
            "     PLAYBACK PaIndex=%d ch=%d name='%s'\n",
            line_number++,
            (int)inputParameters.device,
        ...
        if (status) {
            err = status;
            print_time();
            fprintf(G_fp_logfile, "[%d] main. manage_stream FAILED status=%d\n",
                line_number++, status);
            Audio_Device_Error(err);
        } else {
            print_time();
            fprintf(G_fp_logfile, "[%d] main. Audio stream STARTED OK (I/Q + speaker)\n",
                line_number++);
            fflush(G_fp_logfile);
        }
    }
