# 1 "D:/Microchip/MPLABProjects/PSoC3-PCIe-MKIII.X/cw-gen.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 288 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "D:/Microchip/MPLABProjects/PSoC3-PCIe-MKIII.X/cw-gen.c" 2
...
# 2 "D:/Microchip/MPLABProjects/PSoC3-PCIe-MKIII.X/cw-gen.c" 2
...
# 4 "D:/Microchip/MPLABProjects/PSoC3-PCIe-MKIII.X/cw-gen.c" 2


static long cfg_speed_micros;
static float cfg_speed_wpm;
static long tx_timer;
static uint8_t cw_tx = 0;
static long tx_timer;
static uint8_t tx_flag = 1;
...
void tx_send(long mark) {
  if (tx_flag) do { LATCbits.LATC5 = 1; } while(0);
  tx_timer = mark;
}

void tx_loop(long mark) {
  if(tx_timer - mark < 0) do { LATCbits.LATC5 = 0; } while(0);
}
uint8_t key_read() {
    uint8_t k0,k1;
...
uint8_t key_loop(long mark) {
  ...
  case 3:
    if (start_after - mark < 0) {
      ...
      case 5:
      case 6:
        if (cfg.spacing >= 3) state = 2;
        break;
      }
      if (spacing < 7) spacing += 1;
      if (cfg.mode == 2) state = 3;
      start_after += 1 * cfg_speed_micros;
    }
    break;
  case 4:
  ...
  case 6:
    if (read_after - mark < 0) {
      state = 3;
      staged = 0;
      start_after = mark + 1 * cfg_speed_micros;
      spacing = 0;
      if (mcode & 0x01) {
        mcode = 0xFF;
      } else {
  ...
  if (!staged) {
    if (state > 1) {
      if (k0 && k1) {
        if (ultimatic && cfg.mode == 1) staged = last;
        else if (last == 1) staged = 3;
        else staged = 1;
        ultimatic = 1;
      } else {
        if (k0) staged = 1;
    ...
    else if (!ultimatic || cfg.mode != 1) {
      if (k0 && (last == 3 || spacing > 0)) {
        if (cfg.memory & 2) {
          staged = 1;
          ultimatic = 1;
        }
      }
      if (k1 && (last == 1 || spacing > 0)) {
        if (cfg.memory & 1) {
          staged = 3;
          ultimatic = 1;
        }
  ...
  if (state == 3 && staged) {
    ...
    i += 1 * cfg_speed_micros * (cfg.weight * 2 - 1);
    read_after = start_after = i + (long)cfg.lag * 1000;
    tx_send(start_after);
    ret = 1;
    i += 1 * cfg_speed_micros * (2.0 - cfg.weight * 2);
    if (cfg.spacing >= 1) {
      read_after = i ;
      start_after = i;
    }
    spacing = 0;
    if (mcode & 0x01) {
      if (mcode != 0x01 || staged==3) mcode = 0xFF;
    } else {
      mcode >>= 1;
      if (staged==3) mcode |= 0x80;
    }
    last = staged;
    staged = 0;
    state = 1;
