...
/// <summary>Operator-side MSA1 phones RX + mic TX (WASAPI / WaveIn).</summary>
public sealed class RemoteAfEngine : IDisposable
{
    ...
    public static IReadOnlyList<(int Index, string Name)> PlayDevices => RemotePhonePlayer.ListPlayDevices();
    public static IReadOnlyList<(int Index, string Name)> MicDevices => RemoteMicSender.ListCaptureDevices();
    ...
    public void StartMic(string txHost)
    {
        _mic.Start(txHost, TxPort, MicDeviceIndex);
    }
