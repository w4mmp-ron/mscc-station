...
/* Must match main.c — invalid slot for operator/digital speaker tables */
#ifndef NO_OUTPUT_DEVICE
#define NO_OUTPUT_DEVICE 100
#endif
/* Pan bins: Normal 800 / High 1600 / Max 3200 (CMD_GET_SET_PANADAPTER_REFRESH 0/1/2) */
...
/* PATH_MAX / MAX_PATH: platform.h sets MAX_PATH on Linux; Windows has MAX_PATH */
#if !defined(PATH_MAX)
...
extern char G_digital_audio_device[];
extern char G_audio_device[];
...
/* Call as print_time() or print_time(0) — both OK */
void print_time_impl(void);
#define print_time(...) print_time_impl()
...
extern int Get_Sound_Device(void);
extern int Get_Digital_Sound_Device(void);
extern int Get_IQ_Capture_Device(void);
extern char G_iq_capture_device[PATH_MAX];
extern int IQ_Set_All_Band(int iq_value);
extern void IQ_calc(int iq_int);
extern int update_iq_ini_file(void);
extern int Init_IQ_structure(void);
extern int Load_IQ_Defaults(void); /* no-op on Linux; kept for link compatibility */
