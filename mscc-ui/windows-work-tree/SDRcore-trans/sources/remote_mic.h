/*
 * Remote operator mic: MsccRemotePhones → SDRcore-trans (MSA1 UDP listen).
 *
 * Client sends CMD_SET_AUDIO_DEVICE=2 (REMOTE_AUDIO) when Phones + REMOTE AUDIO.
 * Digital (0) never uses this path. INI: %LOCALAPPDATA%\MSCC-NET9\remote-mic.ini
 * (PORT only; default 9101).
 */
#ifndef REMOTE_MIC_H
#define REMOTE_MIC_H
