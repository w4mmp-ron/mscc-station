...
void Gui_get_param(uint8_t op_code, char *buffer) {
    ...
    switch (op_code) {
        ...
        case CMD_SET_NR:
            /* Live from client: persist NR_VALUE, apply to recv (0 = off, non-zero = level) */
            User_Controls.NR_Value = t_opcode_data;
            SDRcore_recv_send_param(CMD_SET_NR, t_opcode_data);
            User_Controls_Update_Init_File();
            print_time(0);
            fprintf(G_fp_logfile, "[%d] Gui_get_param. CMD_SET_NR → NR_VALUE=%d\n",
...
void * Command_Processor(void *my_param) {
    ...
    while (G_all_threads_run) {
        ...
        case CMD_SET_ALC:
            print_time(0);
            fprintf(G_fp_logfile, "[%d] CMD_SET_ALC. Value: %d\n", line_number++, s_opcode_data);
        ...
        case CMD_GET_SET_SDRCORE_TRANS_VERSION:
            /* Legacy Windows cores still pulse keep-alive via version opcodes. */
        ...
        case CMD_SET_KEEP_ALIVE:
            /*
             * Unified keep-alive opcode 0xF4. Payload selects source (Linux cores):
             ...
             * Windows cores may still use version opcodes (handled above).
             */
