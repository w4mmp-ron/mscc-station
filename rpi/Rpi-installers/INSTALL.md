...
This kit is **`rpi/RRpi-installersers/`**. Ubuntu Desktop (x86_64) is a different path: [`../../INSTALL-UBUNTU.md`](../../INSTALL-UBUNTU.md).
...
2. Contents of **`RRpi-installersers/`** on the Pi (`~/Downloads` or USB).  
...
```bash
cd ~/Downloads/packages    # or wherever you copied Rpi-installers/packages
```
...
**Note:** Rebuild the UI `.deb` after client code changes (`Avalonia-Migration/build-mscc-ui-deb.ps1`), then refresh `RRpi-installersers/` with `collect-packages.ps1`.
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\rpi\Rpi-installers\collect-packages.ps1
```

3. Copy new files from `rpi/RRpi-installersers/` to the Pi.  
4. On the Pi:
...
| Need | Path |
|------|------|
| **This kit + how-to** | `rpi/RRpi-installersers/` |
