...
MSCC_DIR="${MSCC_DIR:-/home/ron/mscc}"
...
need() {
  if [[ ! -x "$MSCC_DIR/$1" ]]; then
    ...
    echo "Build with make from each *-linux worktree (BINDIR=$MSCC_DIR)." >&2
    exit 1
  fi
}
...
usage() {
  ...
  echo "  status  show process state"
