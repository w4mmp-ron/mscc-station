#!/bin/bash
# Install an mscc_*_amd64.deb on Ubuntu Desktop without the scary apt notice:
...
#   ./install-mscc.sh
#   ./install-mscc.sh ./mscc_1.0.42_amd64.deb
#   ./install-mscc.sh --reinstall
#   ./install-mscc.sh --reinstall ./mscc_1.0.42_amd64.deb
#
set -euo pipefail
...
usage() {
  echo "Usage: $0 [--reinstall] [path/to/mscc_VERSION_amd64.deb]" >&2
  exit 1
}
...
pick_deb() {
  ...
  local f
  f="$(ls -1 mscc_*_amd64.deb 2>/dev/null | sort -V | tail -1 || true)"
  if [[ -z "$f" ]]; then
    echo "ERROR: no mscc_*_amd64.deb found in $(pwd)" >&2
    echo "Usage: $0 [--reinstall] [path/to/mscc_VERSION_amd64.deb]" >&2
    exit 1
  fi
  echo "$f"
...
echo "  4. mscc start"
echo "Audio: install mscc-portaudio_*_amd64.deb first (Pulse+ALSA at /usr/local)"
