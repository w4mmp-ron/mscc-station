...
float gaussian()
{
        ...
        rconst1 = (float) (2.0f / (float) RAND_MAX);
        rconst2 = (float) ((float) RAND_MAX / 2.0f);
...
Requires pointer to sp_cplx array, x and power of 2 size of FFT, m
(size of FFT = 2**m).  Places FFT output on top of input sp_cplx array.
...
void fft(sp_cplx *x, int m)
{
        static sp_cplx *w; /* used to store the w sp_cplx array */
        static int mstore = 0; /* stores m for future reference */
        static int n = 1; /* length of fft stored for future */

        sp_cplx u, temp, tm;
        sp_cplx *xi, *xip, *xj, *wptr;
        ...
        if (m != mstore) {
                ...
                w = (sp_cplx *) calloc(le - 1, sizeof(sp_cplx));
                if (!w) {
                        exit(1);
                }
...
void ifft(sp_cplx *x, int m)
{
        ...
        // calc FFT size from order supplied in 'm'
        n = (int) pow(2.0f, (sp_float) m);
        fn = (sp_float) n;
        ...
        for (i = 0; i < n; i++) {
                x[i].real /= fn;
                x[i].imag = -x[i].imag / fn;
        }
}

void dcblock(sp_cplx *samps, state *state, int nsamps)
{
        // remove any DC offset, and high-pass signal at selected lower cutoff
        int j;
        sp_float tempR, tempI;

        static float oldin = 0.0f;
        static float oldout = 0.0f;

        float dcR = 1.0f - (126.0f / state->samplerate); // -3dB @ 20 Hz

        for (j = 0; j < nsamps; j++) {
                tempR = samps[j].real - oldin + dcR * oldout;
                tempI = samps[j].imag - oldin + dcR * oldout;
