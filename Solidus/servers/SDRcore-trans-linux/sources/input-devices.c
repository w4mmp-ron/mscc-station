...
static void fold_alnum(const char *s, char *out, size_t n)
{
    size_t j = 0;
    for (; s && *s && j + 1 < n; s++) {
...
static int folded_hit(const char *name_f, const char *want_f)
{
    size_t n;
    char tmp[48];
...
/* Operator mic: prefer Pulse so ALSA exclusive on the USB phones/mic card
 * does not collide with recv playback. I/Q stays ALSA Proficio. */
static int score_operator_input(const PaDeviceInfo *info, const char *want)
{
    char nf[128], wf[128];
    int score = 0;
    int hit;
    ...
    if (!info || !info->name || !want || !want[0] || info->maxInputChannels < 1)
        return -1;
    ...
    if (strcmp(info->name, want) == 0)
        ...
        score = 4000;
    fold_alnum(info->name, nf, sizeof nf);
    fold_alnum(want, wf, sizeof wf);
    hit = folded_hit(nf, wf);
    if (hit)
        score += 2000 + hit * 10;
    if (score <= 0)
        return -1;
...
void build_input_devices(int device_index) {
    ...
    if (index < MAX_INPUT_DEVICES) {
        if (G_operator_audio_device[0] != '\0' && lpInfo != NULL) {
            int sc = score_operator_input(lpInfo, G_operator_audio_device);
            if (sc > best_op_score) {
                best_op_score = sc;
                G_input_device_index = index;
                print_time();
                fprintf(G_fp_logfile,
                    "[%d] build_input_devices. '%s' BEST score=%d name='%s' pa=%d\n",
                    line_number++, G_operator_audio_device, sc,
                    lpInfo->name, device_index);
            }
        }
