...
/*
 * Si5351 soft-tune strategy (Si570-like):
 *
 * Old approach recomputed Multisynth every step as even(900e6 / fout). That
 * changes MS at fixed clock boundaries (~14.0625 / 28.125 MHz LO-class edges),
 * which matched the observed mid-band pops (e.g. 14.080 / 28.140 with offsets).
 * On LF the same edges occur every ~100 Hz (2200 m) / ~1 kHz (630 m).
 *
 * New approach:
 *  - SOFT: hold Multisynth divider fixed; only rewrite PLL fractional.
 *          Skip MS register writes and SI_PLL_RESET. Valid while VCO stays
 *          in [VCO_MIN, VCO_MAX] and PLL mult in 15..90.
 *  - HARD: pick a new even MS divider targeting ~VCO_CENTER (not the 900 MHz
 *          ceiling), rewrite MS+PLL, then PLL_RESET once.
 *
 * Centering VCO puts ham-band MS edges outside typical amateur segments so
 * a whole band can soft-tune after one hard entry.
 */
...
/* Extra SetFrequency() visits with CLK0 off after hard PLL_RESET.
 * SetFrequency runs ~once per full main beater cycle (~4 ms @ 2 kHz BEAT).
 * 6 ≈ 24 ms post-reset hold (125 ≈ 500 ms was a test — too long for normal use).
 * Soft path never blanks. Does not touch CW timers / CONTROL_RX / PTT. */
