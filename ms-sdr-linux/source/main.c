...
/* mscc.ini PROFICIO-MKII=1 (default) → run MKII PTT sense thread; 0 = legacy */
uint8_t G_proficio_mkii = 1;
...
int32_t G_Rit_Offset = 0; /* signed Hz (was uint32_t — negatives became ~4e9) */
...
int Open_log_file() {
   ...
   if ((homedir = My_getenv("HOME")) != NULL) {
        ...
        if (log_file_dir != NULL) {
            ...
            char* start = strstr(log_file_record, "LOGFILE_DIRECTORY");
            if (start != NULL) {
                start += 18;
                ...
                if (end != NULL) {
                    ...
                    strncat(G_log_file_name, start, size);
        ...
        /* Default log path if no log_file_dir.ini */
        if (G_log_file_name[0] == '\0') {
            snprintf(G_log_file_name, sizeof(G_log_file_name), "%s/ms-sdr.log", homedir);
        }
        /* "w" truncates each run — expected; tail -f will report "file truncated". */
        G_fp_logfile = fopen(G_log_file_name, "w");
        if (G_fp_logfile == NULL) {
            printf("[%d] Open_log_file. File Open Failed: %s\n", line_number++, G_log_file_name);
            return 0;
        }
        /* Line-buffer so tail -f shows output without waiting for 4KB fill. */
...
void print_time(int print_new_line) {
    ...
    if (G_Allow_Log_Write == TRUE) {
        ...
        tim = tv.tv_sec;
        mills = (unsigned short)(tv.tv_usec / 1000);
#else
        {
        ...
        fprintf(G_fp_logfile, "[%02d:%02d:%02d:%02d:%03u]", now->tm_mday, now->tm_hour, now->tm_min, now->tm_sec, (unsigned)mills);
    }
}
...
void* Flusher_thread(void* t) {
    ...
    while (G_all_threads_run) {
        /*
         * Client watchdog is ~10s. Send I'm-Alive (0xF4) every 1s while session
         * is active — independent of log flush / other work.
         */
        if (G_MSCC_Initialized == 1 || G_client_session_active) {
            Gui_send_param(CMD_SET_KEEP_ALIVE, 1);
            if ((ka_log_count++ % 10) == 0) {
                print_time(0);
                fprintf(G_fp_logfile, "[%d] Flusher_thread. Sent CMD_SET_KEEP_ALIVE (0xF4) to client\n",
                    line_number++);
            }
        }
        Sleep(1000);
        ...
        if (G_Allow_Log_Write == TRUE) {
            ...
            /* ms-sdr → cores: real keep-alive opcode (payload = client style value) */
            if (G_network_initialized) {
                SDRcore_recv_send_param(CMD_SET_KEEP_ALIVE, KEEP_ALIVE_FROM_CLIENT);
            ...
            /*
             * Core keep-alive: expect CMD_SET_KEEP_ALIVE (0xF4) with source
             * KEEP_ALIVE_FROM_RECV / KEEP_ALIVE_FROM_TRANS (not version opcodes).
             * Always log missing KA; under full build also stop MSCC.
             */
            if (G_Threads_Started == TRUE) {
                if (servers_count++ >= 10) {
                    print_time(0);
                    fprintf(G_fp_logfile,
                        "[%d] Flusher_thread. core KA status recv=%llu (was %llu) trans=%llu (was %llu)\n",
                        line_number++,
                        (unsigned long long)G_sdrcore_recv_keep_alive,
                        (unsigned long long)previous_sdrcore_recv_keep_alive,
                        (unsigned long long)G_sdrcore_trans_keep_alive,
                        (unsigned long long)previous_sdrcore_trans_keep_alive);
                    fflush(G_fp_logfile);
                    if (G_sdrcore_recv_keep_alive > previous_sdrcore_recv_keep_alive) {
                        ...
                        print_time(0);
                        fprintf(G_fp_logfile,
                            "[%d] Flusher_thread. KEEP-ALIVE missing from sdrcore-recv "
                            "(no 0xF4 source=RECV). count still %llu\n",
                            line_number++,
                            (unsigned long long)G_sdrcore_recv_keep_alive);
                        fflush(G_fp_logfile);
                        servers_failed = 2;
                    ...
                    if (G_sdrcore_trans_keep_alive > previous_sdrcore_trans_keep_alive) {
                        ...
                        print_time(0);
                        fprintf(G_fp_logfile,
                            "[%d] Flusher_thread. KEEP-ALIVE missing from sdrcore-trans "
                            "(no 0xF4 source=TRANS). count still %llu\n",
                            line_number++,
                            (unsigned long long)G_sdrcore_trans_keep_alive);
                        fflush(G_fp_logfile);
                        if (servers_failed == 0)
            ...
            if (servers_failed > 0) {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    fprintf(G_fp_logfile, "[%d] Flusher_thread. Sdrcore-recv FAILED (keep-alive)\n",
                        line_number++);
#if defined(MS_SDR_NO_USB)
                    /* Log only — do not kill bring-up stack */
                    servers_failed = 0;
#else
                    ...
                    break;
                case 3:
                    fprintf(G_fp_logfile, "[%d] Flusher_thread. Sdrcore-trans FAILED (keep-alive)\n",
                        line_number++);
#if defined(MS_SDR_NO_USB)
                    servers_failed = 0;
#else
...
uint16_t Get_band_by_frequency(uint32_t freq, uint8_t apply_offset) {
    uint32_t offset = 0;
    uint16_t band = GENERAL_BAND;
    if (apply_offset) {
        offset = 12000;
    }
    print_time(0);
    fprintf(G_fp_logfile, "[%d] Get_band_by_frequency . Frequency: %ld, Offset: %ld \n", line_number++, freq, offset);
    if (freq >= (1800000 - offset) && freq <= (2000000 + offset)) band = 160;
    if (freq >= 3500000 - offset && freq <= 4000000 + offset) band = 80;
    if (freq >= 5330500 - offset && freq <= 5403500 + offset) band = 60;
    if (freq >= 7000000 - offset && freq <= 7300000 + offset) band = 40;
    if (freq >= 10100000 - offset && freq <= 10150000 + offset) band = 30;
    if (freq >= 14000000 - offset && freq <= 14350000 + offset) band = 20;
    if (freq >= 18068000 - offset && freq <= 18168000 + offset) band = 17;
    if (freq >= 21000000 - offset && freq <= 21450000 + offset) band = 15;
    if (freq >= 24890000 - offset && freq <= 24990000 + offset) band = 12;
    if (freq >= 28000000 - offset && freq <= 29700000 + offset) band = 10;
    if (freq >= 135700 - offset && freq <= 137800 + offset) band = 2200;
    if (freq >= 472000 - offset && freq <= 479000 + offset) band = 630;
    print_time(0);
    fprintf(G_fp_logfile, "[%d] Get_band_by_frequency . Finished . Band: %ld\n", line_number++, band);
    return band;
}
...
void Set_PTT(int on_off, uint8_t allow_MKII_PTT_Switch) {
    ...
    if (G_TX_Inhibit == FALSE) {
        ...
        G_PTT = on_off;
        if (G_PTT) {
            G_spectrum_cycle_count = SPECTRUM_CYCLE_LIMIT;
        }
...
int static TX_Hold_Timer(int period, int reset) {
    int status = FALSE;
    static int end = 0;
...
int SetModeRxTx(int transmit, int tx_delay) {
    ...
    print_time(1);
    fprintf(G_fp_logfile, "[%d] SetModeRxTx Called . transmit %d  gui_running=%d  radio_ok=%d\n",
        line_number++, transmit, (int)G_gui_running, G_transceiver_initialization_status);
    delay_time = delay_time * 1000;
    G_SetModeRxTX = transmit;
    G_dll_active = TRUE;
    /*
     * Key Proficio over USB whenever the radio is available.
     * Previously required G_gui_running (set only on MSCC client Session_Claim),
     * which blocked WSJT / digi PTT in headless operation.
     */
    if (G_transceiver_initialization_status) {
        if (transmit == 1) {
            G_TX = TRUE;
            if (G_Split == FALSE) {
        ...
        if (!G_gui_running) {
            print_time(0);
            fprintf(G_fp_logfile,
                "[%d] SetModeRxTx . headless PTT (no MSCC client session) transmit=%d\n",
                line_number++, transmit);
        }
    } else {
        print_time(0);
        fprintf(G_fp_logfile,
            "[%d] SetModeRxTx . SKIP USB PTT — transceiver not initialized\n",
            line_number++);
    }
    if (ret == FALSE) {
        print_time(0);
...
void ModeChanged(char mode) {
    ...
    if (!G_calibration_mode) {
        ...
        if (status > 0) {
            ...
            /* Still remember requested mode for startup.ini / last-used */
            G_mode = mode;
        }
        ...
        /* Persist mode (and current freq) so next boot matches GUI / CAT mode */
        Update_Startup(G_tune_freq, G_mode);
        print_time(0);
        fprintf(G_fp_logfile, "[%d] ModeChanged. FINISHED (startup.ini MODE=%c)\n",
            line_number++, G_mode);
        G_dll_active = FALSE;
    }
}
...
int static Start_Key_status_thread() {
    int status = TRUE;
    long last_used_t = 0;
...
int static Start_check_temperature_thread() {
    int status = TRUE;
    long last_used_t = 0;
...
int static Start_Message_thread() {
    long t = 0;
    int status = 1;
...
int static Start_Threads() {
    ...
    Sleep(1000);
    /* MKII rear PTT sense (CMD_GET_PTT). Legacy: PROFICIO-MKII=0 in mscc.ini */
    if (G_proficio_mkii) {
        Start_PTT_thread();
        Sleep(1000);
    } else {
...
int main(int argc, char **argv) {
...
#if defined(MS_SDR_NO_USB)
    /* Network / client-handshake testing without radio hardware (WSL default). */
    print_time(0);
    fprintf(G_fp_logfile, "[%d] main . MS_SDR_NO_USB: skipping USB open and firmware checks\n", line_number++);
    G_transceiver_initialization_status = TRUE;
    G_major_version = MULTUS_MAJOR_VERSION_MKII;
    ...
    mode = SI5351_DLL;
    status = Radio_send_parameters(SET_DLL_VERSION, SI5351_DLL, 1);
    Sleep(10);
    print_time(0);
    ...
    Sleep(10);
    print_time(0);
    fprintf(G_fp_logfile, "[%d] main. Calling User_Controls_Init \n", line_number++);
    User_Controls_Init();
    print_time(0);
    ...
    /*
     * Headless: wait briefly for Command_Interface to set G_network_initialized,
     * then push user_controls + startup.ini to cores/rig without a client.
     */
    {
        int wait_i;
        for (wait_i = 0; wait_i < 50 && !G_network_initialized; wait_i++) {
            Sleep(100);
        }
        if (G_network_initialized) {
            print_time(0);
            ...
            print_time(0);
            fprintf(G_fp_logfile,
                "[%d] main. WARNING: network not ready — appliance startup deferred to client connect\n",
                line_number++);
        }
    }
    print_time(0);
