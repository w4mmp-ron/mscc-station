/*
 * MSCC remote operator mic — Windows receiver (MSA1 UDP).
 * Protocol matches MsccRemotePhones TX (default port 9101).
 *
 * Client CMD_SET_AUDIO_DEVICE=2 (REMOTE_AUDIO) selects this mic path.
 * INI supplies PORT only (ENABLED is ignored; kept for old files).
 * Digital (D) ignores this module.
 */
#include "remote_mic.h"
#include "extern.h"
#include "commands.h"

#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define MSA1_MAGIC          0x3141534Du
#define MSA1_HEADER_SIZE    16
#define MSA1_FORMAT_S16LE   0
#define REMOTE_RATE         48000
#define RING_FRAMES         16384 /* @ 48 kHz mono float */
#define MAX_PKT_FRAMES      2048
static int g_port = 9101;
static int g_thread_started;

static SOCKET g_sock = INVALID_SOCKET;
static pthread_t g_thread;
static volatile int g_run;

static float g_ring[RING_FRAMES];
static volatile unsigned g_w;
static volatile unsigned g_r;

static volatile uint64_t g_last_pkt_ms;
static unsigned g_pkt_ok;
static unsigned g_pkt_bad;
static volatile unsigned g_under;

/* 48 kHz ring → 96 kHz: linear interp. g_frac is 48 kHz phase (0..1). */
static float g_hist0, g_hist1, g_frac;

static uint64_t now_ms(void)
{
    return (uint64_t)GetTickCount64();
}

static void ring_write_one(float s)
{
    unsigned w = g_w;
    unsigned next = (w + 1u) % RING_FRAMES;
    if (next == g_r)
        return; /* overrun — drop */
    g_ring[w] = s;
    g_w = next;
}

static float ring_read_one(int *ok)
{
    unsigned r = g_r;
    if (r == g_w) {
        *ok = 0;
        return 0.0f;
    }
    {
        float s = g_ring[r];
        g_r = (r + 1u) % RING_FRAMES;
        *ok = 1;
        return s;
    }
}

static unsigned ring_level(void)
{
    unsigned w = g_w, r = g_r;
    return (w + RING_FRAMES - r) % RING_FRAMES;
}

static void load_config(void)
{
    char path[512];
    FILE *fp;
    char line[256];
    char *homedir;

    g_port = 9101;

    homedir = My_getenv("HOME");
    if (!homedir || !homedir[0]) {
        if (G_fp_logfile) {
            print_time();
            fprintf(G_fp_logfile,
                "[%d] remote_mic: My_getenv failed — using default PORT %d\n",
                line_number++, g_port);
        }
        return;
    }
    CreateDirectoryA(homedir, NULL);
    snprintf(path, sizeof(path), "%s/remote-mic.ini", homedir);

    fp = fopen(path, "r");
    if (!fp) {
        if (G_fp_logfile) {
            print_time();
            fprintf(G_fp_logfile,
                "[%d] remote_mic: no %s — using default PORT %d\n",
                line_number++, path, g_port);
        }
        return;
    }

    while (fgets(line, sizeof(line), fp)) {
        char *eq, *k, *v;
        if (line[0] == '#' || line[0] == '\n' || line[0] == '\r')
            continue;
        eq = strchr(line, '=');
        if (!eq)
            continue;
        *eq = '\0';
        k = line;
        v = eq + 1;
        while (*v == ' ' || *v == '\t')
            v++;
        {
            size_t n = strlen(v);
            while (n > 0 && (v[n - 1] == '\n' || v[n - 1] == '\r' || v[n - 1] == ' '))
                v[--n] = '\0';
        }
        /* ENABLED ignored — mode comes from CMD_SET_AUDIO_DEVICE=2 */
        if (strcmp(k, "PORT") == 0 || strcmp(k, "port") == 0)
            g_port = atoi(v);
    }
    fclose(fp);

    if (g_port <= 0 || g_port > 65535)
        g_port = 9101;
}

static int parse_msa1(const uint8_t *pkt, int len,
    uint16_t *seq, uint16_t *nframes, uint8_t *ch, uint32_t *rate)
{
    uint32_t magic;
    uint16_t frames;
    uint8_t channels, fmt;
    uint32_t sr;
    int need;

    if (len < MSA1_HEADER_SIZE)
        return -1;
    magic = (uint32_t)pkt[0] | ((uint32_t)pkt[1] << 8) |
            ((uint32_t)pkt[2] << 16) | ((uint32_t)pkt[3] << 24);
    if (magic != MSA1_MAGIC)
        return -1;
    *seq = (uint16_t)(pkt[4] | (pkt[5] << 8));
    frames = (uint16_t)(pkt[6] | (pkt[7] << 8));
    channels = pkt[8];
    fmt = pkt[9];
    sr = (uint32_t)pkt[10] | ((uint32_t)pkt[11] << 8) |
         ((uint32_t)pkt[12] << 16) | ((uint32_t)pkt[13] << 24);
    if (channels < 1 || channels > 2)
        return -1;
    if (fmt != MSA1_FORMAT_S16LE)
        return -1;
    if (frames == 0 || frames > MAX_PKT_FRAMES)
        return -1;
    if (sr < 8000)
        return -1;
    need = MSA1_HEADER_SIZE + (int)frames * (int)channels * 2;
    if (len < need)
        return -1;
    *nframes = frames;
    *ch = channels;
    *rate = sr;
    return need;
}

static void *receiver_thread(void *arg)
{
    uint8_t buf[MSA1_HEADER_SIZE + MAX_PKT_FRAMES * 2 * 2];
    (void)arg;

    while (g_run) {
        struct sockaddr_in from;
        int flen = (int)sizeof(from);
        int n = recvfrom(g_sock, (char *)buf, (int)sizeof(buf), 0,
            (struct sockaddr *)&from, &flen);
        uint16_t seq, frames;
        uint8_t ch;
        uint32_t rate;
        int need;
        unsigned i;

        if (!g_run)
            break;
        if (n < 0) {
            Sleep(2);
            continue;
        }
        need = parse_msa1(buf, n, &seq, &frames, &ch, &rate);
        if (need < 0) {
            g_pkt_bad++;
            continue;
        }
        (void)seq;
        /* Prefer 48 kHz; if other rate, still ingest (host should send 48k). */
        {
            const int16_t *pcm = (const int16_t *)(buf + MSA1_HEADER_SIZE);
            for (i = 0; i < frames; i++) {
                int16_t s = (ch >= 2) ? pcm[i * 2u] : pcm[i];
                float f = (float)s / 32768.0f;
                ring_write_one(f);
            }
        }
        g_last_pkt_ms = now_ms();
        g_pkt_ok++;
        if (G_fp_logfile && (g_pkt_ok == 1u || (g_pkt_ok % 500u) == 0u)) {
            float peak = 0.f;
            unsigned j;
            char addrbuf[64];
            const int16_t *pcm2 = (const int16_t *)(buf + MSA1_HEADER_SIZE);
            for (j = 0; j < frames; j++) {
                int16_t s = (ch >= 2) ? pcm2[j * 2u] : pcm2[j];
                float a = (s < 0) ? -(float)s : (float)s;
                if (a > peak) peak = a;
            }
            inet_ntop(AF_INET, &from.sin_addr, addrbuf, sizeof(addrbuf));
            print_time();
            fprintf(G_fp_logfile,
                "[%d] remote_mic: pkt ok=%u bad=%u rate=%u ch=%u frames=%u peak=%.0f occ=%u under=%u from %s\n",
                line_number++, g_pkt_ok, g_pkt_bad, rate, ch, frames, peak,
                ring_level(), g_under, addrbuf);
            g_under = 0;
            fflush(G_fp_logfile);
        }
    }
    return NULL;
}

void remote_mic_init(void)
{
    struct sockaddr_in addr;
    BOOL yes = TRUE;

    g_w = g_r = 0;
    g_sock = INVALID_SOCKET;
    g_run = 0;
    g_thread_started = 0;
    g_last_pkt_ms = 0;
    g_pkt_ok = g_pkt_bad = 0;
    g_under = 0;
    g_hist0 = g_hist1 = 0.0f;
    g_frac = 0.0f;

    load_config();

    g_sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (g_sock == INVALID_SOCKET) {
        if (G_fp_logfile) {
            print_time();
            fprintf(G_fp_logfile,
                "[%d] remote_mic: socket failed: WSA %d\n",
                line_number++, WSAGetLastError());
        }
        return;
    }
    setsockopt(g_sock, SOL_SOCKET, SO_REUSEADDR, (const char *)&yes, sizeof(yes));

    memset(&addr, 0, sizeof(addr));
    addr.sin_family = AF_INET;
    addr.sin_addr.s_addr = htonl(INADDR_ANY);
    addr.sin_port = htons((u_short)g_port);
    if (bind(g_sock, (struct sockaddr *)&addr, sizeof(addr)) == SOCKET_ERROR) {
        if (G_fp_logfile) {
            print_time();
            fprintf(G_fp_logfile,
                "[%d] remote_mic: bind :%d failed: WSA %d\n",
                line_number++, g_port, WSAGetLastError());
        }
        closesocket(g_sock);
        g_sock = INVALID_SOCKET;
        return;
    }

    g_run = 1;
    if (pthread_create(&g_thread, NULL, receiver_thread, NULL) != 0) {
        closesocket(g_sock);
        g_sock = INVALID_SOCKET;
        g_run = 0;
        if (G_fp_logfile) {
            print_time();
            fprintf(G_fp_logfile,
                "[%d] remote_mic: thread create failed\n", line_number++);
        }
        return;
    }
    g_thread_started = 1;

    if (G_fp_logfile) {
        print_time();
        fprintf(G_fp_logfile,
            "[%d] remote_mic: listen UDP :%d (use when CMD_SET_AUDIO_DEVICE=2 or 3)\n",
            line_number++, g_port);
        fflush(G_fp_logfile);
    }
}

void remote_mic_shutdown(void)
{
    if (!g_run && g_sock == INVALID_SOCKET)
        return;
    g_run = 0;
    if (g_sock != INVALID_SOCKET) {
        shutdown(g_sock, SD_BOTH);
        closesocket(g_sock);
        g_sock = INVALID_SOCKET;
    }
    if (g_thread_started) {
        pthread_join(g_thread, NULL);
        g_thread_started = 0;
    }
}

int remote_mic_ready(void)
{
    return g_thread_started && g_run;
}

void remote_mic_fill_stereo_96k(float *stereo_interleaved, unsigned frames)
{
    unsigned i;
    if (!stereo_interleaved || frames == 0)
        return;

    /*
     * 48 kHz mono ring → 96 kHz stereo. Old path held each sample twice and
     * wrote zeros on underrun — that AM/phase-hits a WSJT tone so the SA
     * looks like FM even when drive is low. Linear interp + hold-last, and
     * nudge consume rate so two PC clocks don't click.
     */
    for (i = 0; i < frames; i++) {
        unsigned occ = ring_level();
        float step = 0.5f;
        float s;
        if (occ > (RING_FRAMES * 3u) / 4u)
            step = 0.512f;
        else if (occ > (RING_FRAMES * 5u) / 8u)
            step = 0.504f;
        else if (occ < RING_FRAMES / 8u)
            step = 0.488f;
        else if (occ < RING_FRAMES / 4u)
            step = 0.496f;

        while (g_frac >= 1.0f) {
            int ok = 0;
            g_hist0 = g_hist1;
            g_hist1 = ring_read_one(&ok);
            if (!ok) {
                g_hist1 = g_hist0;
                g_under++;
            }
            g_frac -= 1.0f;
        }
        s = g_hist0 + (g_hist1 - g_hist0) * g_frac;
        stereo_interleaved[i * 2u] = s;
        stereo_interleaved[i * 2u + 1u] = s;
        g_frac += step;
    }
}
