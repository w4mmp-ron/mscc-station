/*
 * MSCC remote operator phones — Windows sender (MSA1 UDP).
    ...
    if (!homedir || !homedir[0]) {
        if (G_fp_logfile) {
            ...
            fprintf(G_fp_logfile,
                "[%d] remote_phones: My_getenv failed (disabled)\n",
                line_number++);
        }
        return;
    }
    CreateDirectoryA(homedir, NULL);
    snprintf(path, sizeof(path), "%s/remote-phones.ini", homedir);
...
static void *sender_thread(void *arg)
{
    ...
    while (g_run) {
        ...
        if (sendto(g_sock, (const char *)packet, (int)sizeof(packet), 0,
                (struct sockaddr *)&g_dest, sizeof(g_dest)) == SOCKET_ERROR) {
            static unsigned err_ct;
            if (G_fp_logfile && (err_ct++ % 100) == 0) {
                print_time();
                fprintf(G_fp_logfile,
                    "[%d] remote_phones: sendto failed: WSA %d\n",
...
void remote_phones_init(void)
{
    ...
    if (g_sock == INVALID_SOCKET) {
        if (G_fp_logfile) {
            print_time();
            fprintf(G_fp_logfile,
                "[%d] remote_phones: socket failed: WSA %d\n",
                line_number++, WSAGetLastError());
        }
        g_enabled = 0;
        return;
