...
int Load_IQ_Defaults() {
    ...
    print_time(0);
    fprintf(G_fp_logfile, "[%d] Load_IQ_Defaults. Called \n", line_number++);
    if ((homedir = My_getenv("HOME")) != NULL) {
        ...
        switch (G_PCB_Version) {
            ...
            case 8:
                ...
                break;
            case 10:
                strcat(l_path, "/rev-MKII/recv-iq.ini");
                break;
        }
        print_time(0);
        fprintf(G_fp_logfile, "[%d] Load_IQ_Defaults. %s\n", line_number++, l_path);
        ...
        if (IQ_Defaults != NULL) {
            ...
            print_time();
            fprintf(G_fp_logfile, "[%d] Load_IQ_Defaults.", line_number);
            ...
            print_time(0);
            fprintf(G_fp_logfile, "[%d] Load_IQ_Defaults. File Open FAILED: %s\n",
                    line_number++, l_path);
        }
    } else {
        print_time();
        fprintf(G_fp_logfile, "[%d] Load_IQ_Defaults. getenv failed\n", line_number++);
    }
    return status;
}
...
int IQ_Set_All_Band(int iq_value) {
    FILE* IQ_update;
    char l_path[MAX_PATH] = { 0 };
    int lenght = 0;
    int band = 0;
    ...
    print_time(0);
    fprintf(G_fp_logfile, "[%d] IQ_Set_All_Band. STARTED. \n", line_number++);
    if ((homedir = My_getenv("HOME")) != NULL) {
        strcpy(l_path, homedir);
        strcat(l_path, "/recv-iq.ini");
        print_time(0);
        fprintf(G_fp_logfile, "[%d] IQ_Set_All_Band. Creating %s \n",
            line_number++, l_path);
        IQ_update = fopen(l_path, "w");
        if (IQ_update != NULL) {
            for (band = 0; band < 12; band++) {
            ...
            print_time(0);
            fprintf(G_fp_logfile, "[%d] IQ_Set_All_Band. Creation Failed: %s\n", line_number++, l_path);
            status = 0;
        }
        print_time(0);
        fprintf(G_fp_logfile, "[%d] IQ_Set_All_Band. Finished \n", line_number++);
        return status;
    }
    return status;
}
...
void IQ_calc(int iq_int) {
    ...
    print_time(0);
    fprintf(G_fp_logfile, "[%d] IQ_calc. STARTED\n",line_number++);
    intval = iq_int;
...
int Init_IQ_structure() {
    ...
    if ((homedir = My_getenv("HOME")) != NULL) {
        ...
        status = 0;
        print_time();
        fprintf(G_fp_logfile, "[%d] Init_IQ_structure. getenv failed\n", line_number++);
    }
    print_time(0);
